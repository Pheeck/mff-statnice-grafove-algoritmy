...
\noindent
\textbf{Verze 4}: Karger-Stein: Fix typo. Tranzitivní uzávěr: Doplněn rozděl a
panuj algoritmus.

\noindent
\textbf{Verze 5}: Minimální kostry: Kontr. borůvka z paralelních hran nechá
vždy \emph{nejlehčí}, ne nejtěžší. Nadpis \enquote{O následujících algoritmech
stačí jen vědět} $rightarrow$ \enquote{Stačí jen vědět, že následující alg.
existují}. Fredman-Tarjan je lin. pro $\exists k: m \in \Omega(n \log^{(k)}
n)$. Union-Find: Má složitost $\OO(\alpha(m, n))$, ne $\OO(\alpha(n, n))$,
odebráno TODO.

\noindent
\textbf{Verze 6}: Dinic na jednotkových vahách: Ty dva algoritmy nemají
$\Theta$ bound na složitost, ale těsný $\OO$ bound.  V tom je vlastně rozdíl.
...
\begin{alg}[Rozděl a panuj zrychlení tranz. uzávěru]
    ~

    \begin{itemize}
        \item $\OO(n^\omega)$
        \item Rozdělíme vstupní matici sousednosti a výstupní matici
            dosažitelnosti na čtyři podmatice a ukážeme, že je vlastně potřeba
            spočítat na každé úrovni rekurze jen dva podproblémy, ne čtyři.
        \item Pak dominantní je kořen stromu rekurze, kde nejvíce času zabere
            násobení (konstantně mnoha) matic $\rightarrow \OO(n^\omega)$.
    \end{itemize}
\end{alg}
...
\begin{anal}[Dinic na jednotkových vahách]
    ...
    \begin{itemize}
        \item $\OO(m^{3 / 2})$
        \item $\OO(\sqrt{n} \cdot m)$ pro $\min(\deg^{in}(v), \deg^{out}(v))
            \le 1$ (těsný bound)
        \item $\OO(n^{2/3} \cdot m)$ pro non-multigrafy (těsný bound)
    \end{itemize}

    (těsnost jsme si nedokazovali)
\end{anal}
...
\begin{note}
    ...
    čísla jako počty hran. Myslím, že algoritmus s tímhle v poho bude
    fungovat.)
\end{note}
...
\begin{alg}[Kontrahující Borůvka]
  Zahazuje smyčky.  Z paralelních hran vždy zahodí všechny kromě nejlehčí
  (red rule).
...
\subsubsection*{Stačí jen vědět, že následující alg. existují}
...
\begin{note}
    ...
    složitosti se pak zjednoduší na $\OO(\log n)$ nebo $\OO(\alpha(m, n))$ za
    operaci.
\end{note}
...
\begin{anal}[Path compression $\OO((n + m) \log n)$]
    ~
