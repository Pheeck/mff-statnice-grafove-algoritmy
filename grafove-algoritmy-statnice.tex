...
\begin{note}
    ...
    nevážený graf (ale Karger-Stein lze upravit pro vážený graf, jenom jsme si
    to na GA1 neukazovali, myslím).
\end{note}
...
\begin{alg}[RMQ$\pm1$]
    ...
    \begin{itemize}
        ...
        \item Makrostruktura si předpočítá nadbloky obsahující počty bloků
            odpovídající mocninám dvojky -- $\OO(n \log n)$ preprocess,
            $\OO(\log^2 n)$ query.
...
\begin{alg-mys}[Verifikace minimality v $O(m)$]
    ...
    hran a pak ještě algoritmus, který je v lineárním čase najde (počkat, není
    to $\OO(m)$)?
\end{alg-mys}
